task :early_childhood_count_data => :environment do
	...
	all_web_after_launch = EarlyLearningData.where(complete: true).where.not(phone_number: invalid_phone_numbers).where("created_at > ?", "2015-08-09")
	puts "All Web After Launch: #{all_web_after_launch.count}"

	all_sms_after_launch = EarlyLearningDataTwilio.where("created_at > ?", "2015-08-09").where(completed: true)
