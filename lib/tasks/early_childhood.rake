task :early_childhood => :environment do
	...
	parsed_data.each do |location|
		...
		# if location["program_information"].include?("CPS Based") && location["program_information"].include?("Head Start") && location["weekday_availability"].include?("Half Day")
		# 	school_based_no_co_pay_half_day.push(location)
		# end
		if location["program_information"].include?("CPS Based") || location["program_information"].include?("CPS Based, Head Start") 
		...
		# if location["zip"].include?(@user.preferred_zipcode) unless @user.other_zipcode == "no"
		# else location["zip"].include?(@user.zipcode)
		# end
	...
	"###"
		puts "This is the count for Home Visiting: #{home_visiting.count}"
