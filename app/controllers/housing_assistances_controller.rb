class HousingAssistancesController < ApplicationController
  before_action :set_housing_assistance, only: [:show, :edit, :update, :destroy]
  ...
  private
    ...
    def set_housing_assistance
      @housing_assistance = HousingAssistance.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def housing_assistance_params
      params.require(:housing_assistance).permit(:housing_dependent_no, :housing_gross_income)
    end
end
